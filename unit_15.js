...
let s1 = new Set().add('h').add('b').add('o').add('h');
console.log(s1);
...
const f7 = () => {
	...
	let s = new Set(i7);
	if (i7.length == s.size && s.size > 6) {
		q = 1;
	}

	document.querySelector('.out-7').textContent = q;
}
...
const f10 = (out_set, elem) => {
	...
	document.querySelector(elem).textContent = q;
 }
